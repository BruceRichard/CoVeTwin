...
if __name__ == "__main__":
    ...
    tmpdir='tmp_mobility/'
    basepath='./PhysX_mobility' # dataset path
    os.makedirs(tmpdir, exist_ok=True)

    indexlist=os.listdir(os.path.join(basepath,'partseg'))
    logger = get_logger('./tmp_mobility/exp_1render_mobility'+str(args.ind)+'.log',verbosity=1)
    logger.info('start')
    ...
    for index in indexlist:
        ...
        with open(os.path.join('tmp_mobility/','finaljson',index+'.json'),'w') as file:
        ...
        for res in resolist:
            ...
            allind = np.empty((0, 3))  
            for nameind in range(len(jsondata['parts'])):
                oripart1=trimesh.Trimesh([])
                for meshname in jsondata['parts'][nameind]['obj']:
                ...
                oripart1.export(os.path.join(savepath,'mesh_new_'+str(nameind)+'.ply'))
                part=o3d.io.read_triangle_mesh(os.path.join(savepath,'mesh_new_'+str(nameind)+'.ply'))
                indices1,vertices1=generate_voxel(part,res)
                alldict_ind[str(nameind)]=indices1
                alldict_vert[str(nameind)]=vertices1

                np.save(os.path.join(savepath,'ind_'+str(nameind)+'.npy'),indices1)
                allind=np.concatenate([allind,indices1],0)
                logger.info(index+'_'+str(nameind)+": "+str(indices1.shape))
