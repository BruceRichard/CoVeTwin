...
from covetwin.voxel_token_codec import (
    VOXEL_COT_PROMPT,
    codec_stats,
    cot_voxel_answer,
)
...
def voxel_encode(voxels: np.ndarray, size: int = 32) -> np.ndarray:
    ...
    assert size == 32, "size must be 32 (2^5)."
...
parser.add_argument("--voxel_codec", type=str, default="auto", choices=["auto", "rle", "box"])
parser.add_argument("--use_geometry_hints", type=int, default=1)
parser.add_argument("--use_physics_hints", type=int, default=1)
parser.add_argument("--output_dir", type=str, default="im_data_obj_sort_new_32_finetune_mobility_v2codec_cot")
...
logger = get_logger(os.path.join(args.output_dir, 'info'+str(args.ind)+'.log'),verbosity=1)
logger.info('start')
...
for name in namelist:
    ...
    for part in range(len(jsondata['parts'])):
        ...
        for ind in range(0,25):
            ...
            dataseq['voxel_codec_stats']=codec_stats(voxeldata)
            ...
            s_dash = cot_voxel_answer(
                voxeldata,
                jsondata,
                part,
                mode=args.voxel_codec,
                use_geometry=bool(args.use_geometry_hints),
                use_physics=bool(args.use_physics_hints),
            )
            question = (
                "Based on the structured description of l_"+str(part)+", generate its 3D voxel grid. "
                + VOXEL_COT_PROMPT
            )
...
with open(os.path.join(args.output_dir,'training_set_'+str(args.ind)+'_randompart_mobility_v2codec_cot.json'), 'w') as json_file:
    json.dump(alldata, json_file, indent=4)   

plt.hist(np.array(alllength), bins=100, color='skyblue')
plt.savefig(os.path.join(args.output_dir, str(args.ind)+'.png'))
