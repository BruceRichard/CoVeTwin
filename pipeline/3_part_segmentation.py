#!/usr/bin/env python3
"""Stage 3: transfer verified coarse labels to the reconstructed fine mesh."""
...
parser = argparse.ArgumentParser(
    description="Transfer CoVeTwin coarse labels to high-resolution part meshes."
)
parser.add_argument("--index", type=int, default=0)
parser.add_argument("--range", type=int, default=2000)
parser.add_argument("--basepaths", nargs="+", default=["./test_demo_new"])
...
for basepath in args.basepaths:
    ...
    for name in namelist:
        ...
        if os.path.exists(os.path.join(tmpdir,'sample.glb')):
            ...
            logger.info('complete: '+basepath+'/'+name)
        else:
            logger.info('skip: '+basepath+'/'+name)
